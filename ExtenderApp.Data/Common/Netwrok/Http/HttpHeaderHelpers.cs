...
namespace ExtenderApp.Data
{
    ...
    /// 把有关 Host / Content-WrittenCount / 可选默认 Content-Type 的规则集中到这里，避免分散在各处。
    /// </summary>
    public static class HttpHeaderHelpers
    {
        ...
        /// - 如果 body 非空且没有 Content-WrittenCount 则补上 Content-WrittenCount；
        ...
        public static void EnsureRequestHeaders(this HttpHeader headers, Uri? requestUri, in ByteBlock body, string? defaultContentType = null)
        {
            ...
            // Content-WrittenCount：当有 body 且未设置时补齐
            if (body.WrittenCount > 0 && !headers.ContainsHeader(HttpHeaders.ContentLength))
            {
                headers.SetValue(HttpHeaders.ContentLength, body.WrittenCount.ToString());
            }

            // 默认 Content-Type（仅在 body 存在且未显式设置时生效）
            if (!string.IsNullOrEmpty(defaultContentType) && body.WrittenCount > 0 && !headers.ContainsHeader(HttpHeaders.ContentType))
            {
        ...
        /// <summary>
        /// 在序列化响应之前确保头部已被补齐（示例：Content-WrittenCount、Date 等）。
        /// </summary>
