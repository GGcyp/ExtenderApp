...
namespace ExtenderApp.Data
{
    ...
    /// </summary>
    public readonly struct ExtenderAwaiter : ICriticalNotifyCompletion
    {
        /// <summary>
        /// 取消令牌，用于支持异步操作的取消。
        /// </summary>
        ...
        public ExtenderAwaiter(Action<Action>? callback, Action<Action>? unsafeCallback, Func<bool>? isCompletedFunc, CancellationToken token)
        {
            _token = token;
            _callback = callback;
        ...
        /// <param name="token">取消令牌，将在 <see cref="GetResult"/> 处传播取消。</param>
        public ExtenderAwaiter(Action<Action>? callback, Action<Action>? unsafeCallback, bool isCompleted, CancellationToken token)
        {
            _token = token;
            _callback = callback;
            _unsafeCallback = unsafeCallback;
