namespace ExtenderApp.Data
{
    ...
    /// 通常与 <see cref="ExtenderAwaiter"/> 配合使用，封装异步调度、主线程切换等场景。
    /// 典型用法：await dispatcher.ToMainThreadAsync(); // 之后的代码在线程上下文（如 UI 线程）执行
    /// </summary>
    /// <seealso cref="ExtenderAwaiter"/>
    public readonly struct ExtenderAwaitable
    {
        /// <summary>
        /// 取消令牌，用于支持异步操作的取消，在 <see cref="ExtenderAwaiter.GetResult"/> 处传播取消。
        /// </summary>
        ...
        public ExtenderAwaitable(Action<Action>? callback, Action<Action>? unsafeCallback, bool isCompleted, CancellationToken token = default)
        {
            _token = token;
            _callback = callback;
        ...
        /// <param name="token">取消令牌。</param>
        public ExtenderAwaitable(Action<Action>? callback, Action<Action>? unsafeCallback, Func<bool>? isCompletedFunc, CancellationToken token = default)
        {
            _token = token;
            _callback = callback;
            _unsafeCallback = unsafeCallback;
        ...
        /// </summary>
        /// <returns><see cref="ExtenderAwaiter"/> 实例。</returns>
        /// <remarks>
        /// 若提供了 <see cref="_isCompletedFunc"/>，则走“动态判断”路径；否则使用固定完成态 <see cref="_isCompleted"/>。
        /// </remarks>
        public ExtenderAwaiter GetAwaiter()
        {
            if (_isCompletedFunc != null)
                return new ExtenderAwaiter(_callback, _unsafeCallback, _isCompletedFunc, _token);

            return new ExtenderAwaiter(_callback, _unsafeCallback, _isCompleted, _token);
        }
    }
}
