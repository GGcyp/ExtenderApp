...
namespace ExtenderApp.Data
{
    ...
    public unsafe struct NativeByteMemory : IDisposable, IEquatable<NativeByteMemory>
    {
        ...
        /// <summary>
        /// 从 <see cref="ReadOnlySpan{T}"/> 向此 <see cref="NativeByteMemory"/> 写入数据。
        /// </summary>
        /// <param name="source">要写入的数据源。</param>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        ...
        /// <summary>
        /// 从指针向此 <see cref="NativeByteMemory"/> 写入数据。
        /// </summary>
        /// <param name="source">指向源数据的指针。</param>
        ...
        public void Write(byte* source, int length)
        {
            ...
            if (length < 0 || length > Length)
                throw new ArgumentOutOfRangeException(nameof(length));
            if (length == 0) return;
            new Span<byte>(source, length).CopyTo(Span);
        }

        /// <summary>
        /// 从另一个 <see cref="NativeByteMemory"/> 向此 <see cref="NativeByteMemory"/> 写入数据。
        /// </summary>
        /// <param name="source">要写入的源 <see cref="NativeByteMemory"/>。</param>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void Write(NativeByteMemory source)
        {
            source.Span.CopyTo(Span);
        }

        /// <summary>
        /// 从另一个 <see cref="NativeByteMemory"/> 向此 <see cref="NativeByteMemory"/> 写入指定长度的数据。
        /// </summary>
        /// <param name="source">要写入的源 <see cref="NativeByteMemory"/>。</param>
        /// <param name="length">要写入的字节数。</param>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void Write(NativeByteMemory source, int length)
        {
            if (length < 0 || length > source.Length || length > Length)
                throw new ArgumentOutOfRangeException(nameof(length));
            if (length == 0) return;
            source.Span.Slice(0, length).CopyTo(Span);
        }

        /// <summary>
        /// 从 <see cref="ReadOnlyMemory{T}"/> 向此 <see cref="NativeByteMemory"/> 写入数据。
        /// </summary>
        /// <param name="source">要写入的数据源。</param>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
