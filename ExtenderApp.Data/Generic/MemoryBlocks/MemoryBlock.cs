...
namespace ExtenderApp.Data
{
    ...
    /// - WrittenCount 表示已写入元素的数量（写指针/写边界）。
    ...
    public struct MemoryBlock<T>
    {
        ...
        /// <summary>
        /// 剩余未读数据的数量（WrittenCount - Consumed）。
        /// </summary>
        public int Remaining => WrittenCount - Consumed;
        ...
        /// <summary>
        /// 可写入的数量（Capacity - WrittenCount）。确保不为负数。
        /// </summary>
        public int WritableBytes => Capacity >= WrittenCount ? Capacity - WrittenCount : 0;
        ...
        /// <summary>
        /// 获取已写入范围的只读 ArraySegment。
        /// </summary>
        public ArraySegment<T> UnreadSegment
        {
            get
            {
                if (array == null)
                    return new ArraySegment<T>(Array.Empty<T>(), 0, 0);
                return new ArraySegment<T>(array, Consumed, Remaining);
            }
        }
        ...
        public MemoryBlock(ReadOnlySpan<T> span, ArrayPool<T> pool) : this(span.Length, pool)
        {
            ...
            span.CopyTo(array);
            WrittenCount = span.Length;
        }

        /// <summary>
        /// 复制构造：基于另一个块的内容创建新实例（租用同等容量并拷贝已写入数据）。
        /// </summary>
        /// <param name="block">源块（仅拷贝已写入范围 0..WrittenCount）。</param>
        public MemoryBlock(in MemoryBlock<T> block)
        {
            ...
            WrittenCount = block.WrittenCount;
            array = _pool.Rent(block.Capacity);
            Array.Copy(block.array!, array, WrittenCount);
        }

        /// <summary>
        /// 获取用于写入的可写内存切片。切片起点为当前 <see cref="WrittenCount"/>。
        /// </summary>
        ...
        {
            Ensure(sizeHint);
            return array.AsMemory(WrittenCount);
        }

        /// <summary>
        /// 获取用于写入的可写 UnreadSpan。切片起点为当前 <see cref="WrittenCount"/>。
        /// </summary>
        /// <param name="sizeHint">预计需要写入的最小容量提示，用于触发扩容。</param>
        ...
        {
            Ensure(sizeHint);
            return array.AsSpan(WrittenCount);
        }
        ...
        /// <summary>
        /// 将可读数据（WrittenCount - Consumed）复制到目标 UnreadSpan。
        /// </summary>
        ...
        {
            int length = WrittenCount - Consumed;
        ...
        {
            int newCapacity = WrittenCount + sizeHint;
            ...
            if (WrittenCount > 0)
            {
                Array.Copy(oldArray, 0, array, 0, WrittenCount);
            }
            _pool.Return(oldArray);
        }

        /// <summary>
        /// 将读指针设置为绝对位置 <paramref name="count"/>。范围：0..WrittenCount。
        /// </summary>
        ...
        {
            if (start < Consumed || start > WrittenCount)
        ...
        public void Clear()
        {
            if (WrittenCount > 0)
            {
                Array.Clear(array, 0, WrittenCount);
            }
        }
        ...
        public void Compact()
        {
            ...
            if (array == null || WrittenCount == 0 || Consumed == 0)
                return;

            int unread = WrittenCount - Consumed;
            if (unread <= 0)
            {
                // 全部消费，重置指针以重用整个缓冲
                Reset();
        ...
        /// <summary>
        /// 复制剩余未读数据（WrittenCount - Consumed）为新数组。
        /// </summary>
        ...
        /// </summary>
        /// <returns>包含 [0..WrittenCount) 范围内数据的新数组；若无已写入数据则返回 <see cref="Array.Empty{T}"/>。</returns>
        public T[] ToAllArray()
        {
            if (WrittenCount == 0 || array is null)
                return Array.Empty<T>();

            var newArray = new T[WrittenCount];
            Array.Copy(array, newArray, WrittenCount);
            return newArray;
        }
        ...
        public MemoryBlock<T> Clone() => new MemoryBlock<T>(UnreadSpan);
        ...
        {
            Ensure(span.Length);
            span.CopyTo(array.AsSpan(WrittenCount));
        ...
        public void Write(in ReadOnlySequence<T> value)
        {
            ...
            {
                segment.Span.CopyTo(array.AsSpan(WrittenCount));
        ...
        /// <summary>
        /// 读取最多 destination.WrittenCount 个未读元素到目标跨度，并将读指针前进相应长度（不影响写指针）。
        /// </summary>
        /// <param name="destination">接收数据的目标跨度。</param>
        /// <returns>实际读取的元素数量。</returns>
        /// <exception cref="InvalidOperationException">当没有可读数据（Remaining == 0）时抛出。</exception>
        /// <remarks>读取数量为 Math.Min(destination.WrittenCount, Remaining)。</remarks>
        public int Read(Span<T> destination)
        {
            if (Remaining <= 0)
                throw new InvalidOperationException("没有可读数据。");

            int length = Math.Min(destination.Length, Remaining);
            UnreadSpan.Slice(0, length).CopyTo(destination);
            ReadAdvance(length);
            return length;
        }

        /// <summary>
        /// 读取最多 destination.WrittenCount 个未读元素到目标内存，并将读指针前进相应长度（不影响写指针）。
        /// </summary>
        /// <param name="destination">接收数据的目标内存。</param>
        /// <returns>实际读取的元素数量。</returns>
        /// <exception cref="InvalidOperationException">当没有可读数据（Remaining == 0）时抛出。</exception>
        /// <remarks>等同于调用 Read(destination.Span)。实际读取数量为 Math.Min(destination.WrittenCount, Remaining)。</remarks>
        ...
        /// 读取指定数量的元素，并推进读指针。
        /// </summary>
        /// <param name="length">指定读取的元素数量。</param>
        /// <returns>返回读取到的元素切片。</returns>
        /// <exception cref="InvalidOperationException">当没有可读数据（Remaining == 0）时抛出。</exception>
        /// <exception cref="ArgumentOutOfRangeException">当读取长度小于 0 或大于剩余可读长度时抛出。</exception>
