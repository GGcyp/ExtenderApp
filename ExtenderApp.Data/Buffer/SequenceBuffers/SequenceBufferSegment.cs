...
namespace ExtenderApp.Data.Buffer.Sequence
{
    ...
    public abstract class SequenceBufferSegment<T> : ReadOnlySequenceSegment<T>, IPinnable
    {
        ...
        /// <summary>
        /// 回收从指定段开始的整个段链（当前实现仅调用 Release，具体回收行为由派生类实现）。调用方应确保被回收的段不再被使用。
        /// </summary>
        /// <param name="segment">指定段</param>
        public static void ReleaseSegment(SequenceBufferSegment<T>? segment)
        {
            while (segment != null)
            {
        ...
        /// <summary>
        /// 派生类提供具体的推进实现。实现应将当前段的已提交长度向前推进指定数量，并在必要时触发相关事件或状态更新以反映写入进度。
        /// </summary>
