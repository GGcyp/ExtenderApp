...
namespace ExtenderApp.Data.Buffer
{
    ...
    public struct SequenceBufferReader<T> : IDisposable, IEquatable<SequenceBufferReader<T>> where T : unmanaged, IEquatable<T>
    {
        private readonly SequenceBuffer<T> _buffer;
        ...
        public SequenceBufferReader(SequenceBuffer<T> buffer)
        {
        ...
        /// </summary>
        public long Remaining => _buffer.Committed - Consumed;
        ...
        public ReadOnlySequence<T> UnreadSequence
        {
            get
            {
                ...
                return new ReadOnlySequence<T>(currentSegment, segmentConsumed, _buffer.Last ?? SequenceBufferSegment<T>.Empty, (int)(_buffer.Last?.Committed ?? 0));
            }
        }
        ...
        {
            if (count < 0 || Consumed + count > _buffer.Committed)
        ...
        public bool Equals(SequenceBufferReader<T> other)
        {
            return _buffer == other._buffer && Consumed == other.Consumed;
        }

        public static bool operator ==(SequenceBufferReader<T> left, SequenceBufferReader<T> right)
            => left.Equals(right);
        ...
        public override bool Equals(object? obj)
            => obj is SequenceBufferReader<T> other && Equals(other);

        public override int GetHashCode()
            => HashCode.Combine(_buffer, Consumed);
        ...
        public static implicit operator SequenceBufferReader<T>(SequenceBuffer<T> buffer)
            => new SequenceBufferReader<T>(buffer);

        public static implicit operator ReadOnlySequence<T>(SequenceBufferReader<T> reader)
            => reader.UnreadSequence;

        public static implicit operator SequenceReader<T>(SequenceBufferReader<T> reader)
        {
