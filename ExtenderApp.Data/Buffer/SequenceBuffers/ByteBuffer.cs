...
{
    /// <summary>
    /// 基于 <see cref="SequenceBuffer{byte}"/> 的轻量字节写入/读取包装器。 提供高性能的写入（通过 <see cref="IBufferWriter{byte}"/>）与读取视图（通过内部的 <see cref="SequenceBufferReader{byte}"/>），并支持池化场景的便捷构造与释放。
    /// </summary>
    /// <remarks>
    ...
    public struct ByteBuffer : IBufferWriter<byte>, IDisposable
    {
        ...
        public ByteBuffer() : this(SequenceBufferPool<byte>.Shared)
        {
        }
        ...
        /// <summary>
        /// 从指定的 <see cref="SequenceBufferPool{byte}"/> 租用一个缓冲并创建 <see cref="ByteBuffer"/>。
        /// </summary>
        /// <param name="pool">用于租用缓冲区的池。</param>
        public ByteBuffer(SequenceBufferPool<byte> pool) : this(pool.Rent())
        {
        }

        /// <summary>
        /// 使用已有的 <see cref="SequenceBuffer{byte}"/> 创建包装器。构造时会对传入缓冲调用 <see cref="SequenceBuffer{T}.Freeze"/>。
        /// </summary>
        ...
        /// <summary>
        /// 从指定的 <see cref="SequenceBufferPool{byte}"/> 隐式租用缓冲并创建 <see cref="ByteBuffer"/>。
        /// </summary>
        /// <param name="pool">缓冲区池。</param>
        public static implicit operator ByteBuffer(SequenceBufferPool<byte> pool)
            => new ByteBuffer(pool);
