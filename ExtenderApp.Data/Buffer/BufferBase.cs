...
namespace ExtenderApp.Data.Buffer
{
    ...
    public abstract class BufferBase<T> : FreezeObject, IBufferWriter<T>, IPinnable
    {
        ...
        /// <summary>
        /// 将另一个 <see cref="BufferBase{T}"/> 的已提交数据写入当前缓冲区（等效于写入其 <see cref="CommittedSequence"/>）。
        /// </summary>
        /// <param name="other">来源缓冲区，其已提交数据将被复制到当前缓冲区。</param>
        /// <remarks>此方法不会修改来源缓冲区的状态。调用方应遵循两端的生命周期约定以避免并发修改问题。</remarks>
        public void Write(BufferBase<T> other)
        {
        ...
        public MemoryHandle Pin(int elementIndex)
        {
            ...
            isPinned = true;
            return memoryHandle = PinProtected(elementIndex);
        }
        ...
        public static implicit operator ReadOnlySequence<T>(BufferBase<T> buffer) => buffer.CommittedSequence;
    }
}