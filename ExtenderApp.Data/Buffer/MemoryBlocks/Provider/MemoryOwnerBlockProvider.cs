...
namespace ExtenderApp.Data.Buffer.MemoryBlocks
{
    ...
    public sealed class MemoryOwnerBlockProvider<T> : MemoryBlockProvider<T>
    {
        ...
        protected override MemoryBlock<T> CreateBufferProtected(int sizeHint)
        {
            ...
            block.MemoryOwner = sizeHint > 0 ? _memoryPool.Rent(sizeHint) : _memoryPool.Rent(DefaultInitialBlockSize);
        ...
        {
            public ObjectPool<MemoryOwnerMemoryBlock> BlockPool;
            ...
            /// <summary>
            /// 释放/回收内存块的保护实现：释放并置空内部的 <see cref="IMemoryOwner{T}"/>，以便归还给内存池并允许对象池复用此实例。
            /// </summary>
