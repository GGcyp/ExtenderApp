...
namespace ExtenderApp.Abstract
{
    ...
    public interface ILinkSender
    {
        ...
        /// <returns>一个包含 <see cref="SocketOperationValue"/> 的 <see cref="Result{T}"/> 实例，语义与 <see cref="Send(Memory{byte})"/> 一致。</returns>
        ...
        /// <returns>一个包含 <see cref="SocketOperationValue"/> 的 <see cref="Result{T}"/> 实例，语义与 <see cref="Send(Memory{byte})"/> 一致。</returns>
        Result<SocketOperationValue> Send(ReadOnlySpan<byte> span, LinkFlags flags);
        ...
        /// cref="SocketOperationValue.BytesTransferred"/> 获取实际发出的字节数。 <br/>- 失败时， <see cref="Result.Exception"/> 包含相应的网络或逻辑异常信息。
        /// </returns>
        Result<SocketOperationValue> Send(ReadOnlyMemory<byte> memory);
        ...
        /// cref="SocketOperationValue.BytesTransferred"/> 获取实际发出的字节数。 <br/>- 失败时， <see cref="Result.Exception"/> 包含相应的网络或逻辑异常信息。
        /// </returns>
        Result<SocketOperationValue> Send(ReadOnlyMemory<byte> memory, LinkFlags flags);
        ...
        /// <param name="flags">发送标志。</param>
        /// <returns>一个包含 <see cref="SocketOperationValue"/> 的 <see cref="Result{T}"/> 实例。</returns>
        Result<SocketOperationValue> Send(IList<ArraySegment<byte>> buffer, LinkFlags flags);
        ...
        /// </returns>
        ValueTask<Result<SocketOperationValue>> SendAsync(ReadOnlyMemory<byte> memory, CancellationToken token = default);
        ...
        /// cref="OperationCanceledException"/> 的失败 <see cref="Result"/>。
        /// </returns>
        ValueTask<Result<SocketOperationValue>> SendAsync(ReadOnlyMemory<byte> memory, LinkFlags flags, CancellationToken token = default);
        ...
        /// <param name="token">用于取消异步操作的令牌。</param>
        /// <returns>一个表示异步操作的 <see cref="ValueTask{TResult}"/>。其结果是一个 <see cref="Result{T}"/>，语义与同步 Scatter 发送方法一致。</returns>
        ValueTask<Result<SocketOperationValue>> SendAsync(IList<ArraySegment<byte>> buffer, LinkFlags flags, CancellationToken token = default);
