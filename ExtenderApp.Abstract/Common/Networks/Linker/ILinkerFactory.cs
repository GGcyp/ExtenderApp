...
namespace ExtenderApp.Abstract
{
    ...
    {
        ProtocolType LinkerProtocolType { get; }
        SocketType LinkerSocketType { get; }
        ILinker CreateLinker();
        ILinker CreateLinker(Socket socket);
        ILinker CreateLinker(AddressFamily addressFamily);
    }

    /// <summary>
    /// 链接器工厂接口
    /// </summary>
    public interface ILinkerFactory<T> : ILinkerFactory where T : ILinker
    {
        /// <summary>
        ...
        new T CreateLinker(Socket socket);
        ...
        /// <returns>返回指定类型的链接器实例。</returns>
        new T CreateLinker(AddressFamily addressFamily);
    }
}
