...
{
    /// <summary>
    /// 链路客户端消息打包 / 解包 器接口。 提供将应用层消息封帧为传输帧，以及从接收字节流中解析完整帧的能力。
    /// </summary>
    public interface ILinkClientFramer : IDisposable
    {
        ...
        void Encode(ref FrameContext framedMessage);
        ...
        /// </summary>
        /// <param name="magic">要设置的魔数字节序列。传入长度为 0 或空视为禁用魔数检查（语义由实现定义）。 注意：参数以 <see cref="ReadOnlySpan{T}"/> 形式传入，调用者可传入栈分配或堆数据。</param>
