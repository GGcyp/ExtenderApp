...
namespace ExtenderApp.Abstract
{
    ...
    /// 提供数据压缩与解压的抽象接口。
    /// </summary>
    /// <remarks>实现应尽量避免不必要的数据拷贝并在可能时直接将结果写入目标缓冲。 所有方法均应在失败时尽量返回 <c>false</c>，而非抛出异常，除非遇到严重错误。</remarks>
    public interface ICompression
    {
        ...
        /// <remarks>实现应尽量避免分配，优先在目标缓冲中直接写入压缩数据以减少拷贝开销。</remarks>
        ...
        /// <param name="compressionType">指定使用的压缩类型或模式，默认为 <see cref="CompressionType.Block"/>。</param>
        /// <returns>若压缩成功则返回 <c>true</c> 并通过 <paramref name="output"/> 返回结果；否则返回 <c>false</c>。</returns>
        bool TryCompress<TBuffer>(TBuffer input, out AbstractBuffer<byte> output, CompressionType compressionType = CompressionType.Block) where TBuffer : AbstractBuffer<byte>;
        ...
        /// <returns>若解压成功则返回 <c>true</c> 并通过 <paramref name="output"/> 返回结果；否则返回 <c>false</c>。</returns>
        bool TryDecompress<TBuffer>(TBuffer input, out AbstractBuffer<byte> output) where TBuffer : AbstractBuffer<byte>;
    }
}