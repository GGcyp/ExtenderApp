...
{
    /// <summary>
    /// 定义对本地文件的同步/异步读写、容量扩展与元信息访问的契约。 实现者应保证在失败时返回相应的 <see cref="Result"/>/ <see cref="Result{T}"/>， 在成功时更新 <see cref="LastOperateTime"/> 并返回实际读取/写入的字节数或数据。
    /// </summary>
    public interface IFileOperate : IDisposable
    {
        ...
        /// <summary>
        /// 获取或设置是否由外部宿主管理此实例（生命周期/资源）。 若为 <c>true</c>，调用方负责释放资源；若为 <c>false</c>，实现者应负责生命周期管理。
        /// </summary>
        ...
        /// <returns>表示操作结果的 <see cref="Result{T}"/>。成功时 <see cref="Result{T}.Value"/> 为实际写入的字节数（int）。 失败时返回包含错误消息或异常的 <see cref="Result{T}"/>。</returns>
        ...
        /// <returns>表示操作结果的 <see cref="Result{T}"/>。成功时 <see cref="Result{T}.Value"/> 为实际写入的字节数（int）。</returns>
        ...
        /// <returns>表示异步操作的 <see cref="ValueTask{Result{T}}"/>，其结果为 <see cref="Result{T}"/>。 成功时 <see cref="Result{T}.Value"/> 为实际写入的字节数（int）。</returns>
