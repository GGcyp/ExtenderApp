...
namespace ExtenderApp.Abstract
{
    ...
    public interface ILinkInfo
    {
        ...
        /// </summary>
        /// <remarks>- 对于尚未绑定的套接字可能为 null；实现应在可用时返回实际 <see cref="EndPoint"/>。</remarks>
        EndPoint? LocalEndPoint { get; }

        /// <summary>
        /// 远端终结点（对端地址与端口），若未确定则为 null。
        /// </summary>
        /// <remarks>- 对于已 Connect 的 socket（或面向连接协议）应返回对端地址；对未 Connect 的 UDP 可能为 null。</remarks>
        ...
        /// </summary>
        /// <remarks>
        /// - 不应返回 null；实现可根据需要暴露全局或 per-link 的限流器。 <br/>
        /// - CapacityLimiter 是线程安全的，调用方应使用其 Lease/Acquire API 在发送/接收前申请容量并在完成后释放，避免超限。
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// - ValueCounter 为线程安全类型；通过 <see cref="ValueCounter.Increment(long)"/> 累计已发送字节数或消息数。 <br/>
        ...
        /// </summary>
        /// <remarks>- 语义与 <see cref="SendCounter"/> 对应，用于累计接收字节数或消息数并提供周期性统计信息。</remarks>
