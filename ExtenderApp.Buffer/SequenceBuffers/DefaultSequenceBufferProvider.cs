...
namespace ExtenderApp.Buffer
{
    ...
    public class DefaultSequenceBufferProvider<T> : SequenceBufferProvider<T>
    {
        private static readonly Lazy<DefaultSequenceBufferProvider<T>> _default = new(() => new());

        /// <summary>
        /// 全局共享的序列池单例，便于在应用中复用同一池实例。
        /// </summary>
        public static readonly DefaultSequenceBufferProvider<T> Default = _default.Value;

        private readonly ObjectPool<DefaultSequenceBuffer> _objectPool =
            ObjectPool.Create(static () => new DefaultSequenceBuffer());
        ...
        protected override void ReleaseProtected(SequenceBuffer<T> buffer)
        {
            ...
            if (buffer is DefaultSequenceBuffer defaultBuffer)
            {
                _objectPool.Release(defaultBuffer);
            }
            else
            {
                buffer.Dispose();
        ...
        private class DefaultSequenceBuffer : SequenceBuffer<T>
        {
            public DefaultSequenceBuffer() : base()
            {
            }
        }
    }
