...
namespace ExtenderApp.Buffer
{
    ...
    /// - <see cref="Committed"/> 表示已写入字节数（写指针/写边界）。
    /// - 通过内部 <see cref="ValueMemoryBlockReader{T}"/> 保持读指针位置（ <see cref="Consumed"/>）。 注意：此类型并非线程安全；使用完毕需调用 <see cref="Dispose"/> 归还底层资源（视提供者而定）。
    /// </summary>
    public struct ByteBlock : IDisposable, IEquatable<ByteBlock>
    {
        ...
        /// <summary>
        /// 使用已有的 <see cref="MemoryBlock{byte}"/> 初始化（读指针重置为起点）。
        /// </summary>
        ...
        /// <summary>
        /// 将读指针设置为绝对位置（0..Committed）。通常用于回退或定位读取位置。
        /// </summary>
        /// <param name="count">新的读指针位置（相对于已写入起点）。</param>
        /// <exception cref="ArgumentOutOfRangeException">当位置不在合法范围内时抛出。</exception>
        public void Rewind(int count) => Block.Rewind(count);

        /// <summary>
        /// 清零已使用区域的数据并重置读取器（不改变写指针，但会清除存储的内容以便 GC 回收）。
        /// </summary>
        public void Clear()
        {
            Block.Clear();
        ...
        /// <summary>
        /// 复制剩余未读数据为新数组（会分配），不改变读/写指针。
        /// </summary>
        /// <returns>包含未读数据的新数组；若无未读数据则返回空数组。</returns>
        public byte[] ToArray() => Block.ToArray();

        /// <summary>
        /// 复制当前已写入的数据为新数组（范围 [0..Committed)），包含已消费部分，不改变读/写指针。
        /// </summary>
        /// <returns>包含 [0..Committed) 范围内数据的新数组；若无已写入数据则返回空数组。</returns>
        public byte[] ToAllArray() => Block.Memory.ToArray();

        /// <summary>
        /// 将从读指针到写指针范围内的字节顺序整体反转（不改变读/写指针）。
        /// </summary>
        public void Reverse()
        {
            Block.Reverse();
        ...
        /// <summary>
        /// 反转指定区间内的字节顺序（相对于已写入区域），不改变读写指针。
        /// </summary>
        /// <param name="start">开始索引（相对于已写入起点，不能早于已消费位置）。</param>
        /// <param name="length">要反转的字节数（不能超过未读取的字节数）。</param>
        ...
        /// <summary>
        /// 归还底层缓冲到提供者/数组池并释放读取器占用的引用。调用后不应再使用此实例（只读切片将失效）。
        /// </summary>
        public void Dispose()
        {
            Block.TryRelease();
        ...
        /// <summary>
        /// 基于未读数据计算并返回一个用于集合键的哈希码（使用 SHA256 并取前 4 字节转换为 int）。
        /// </summary>
        /// <returns>基于未读数据的哈希码。</returns>
        ...
        public static bool operator ==(ByteBlock left, ByteBlock rigth) => left.Equals(rigth);
        ...
        /// <summary>
        /// 将另一个 <see cref="ByteBlock"/> 的未读数据写入当前块（按 <see cref="CommittedSpan"/> 复制），并推进写指针。
        /// </summary>
        ...
        /// <summary>
        /// 将另一个 <see cref="MemoryBlock{byte}"/> 的已提交/未读数据写入当前块（按其 <see cref="MemoryBlock{T}.CommittedMemory"/> 复制），并推进写指针。
        /// </summary>
        ...
        /// <summary>
        /// 隐式将 <see cref="ByteBlock"/> 转换为其未读字节的 <see cref="ReadOnlySpan{byte}"/>。
        /// </summary>
        /// <param name="block">源 <see cref="ByteBlock"/>。</param>
        ...
        /// <summary>
        /// 隐式将 <see cref="ByteBlock"/> 转换为其未读字节的 <see cref="ReadOnlyMemory{byte}"/>。
        /// </summary>
        /// <param name="block">源 <see cref="ByteBlock"/>。</param>
        ...
        /// <summary>
        /// 隐式将 <see cref="ByteBlock"/> 转换为其未读字节的 <see cref="ArraySegment{byte}"/>.
        /// </summary>
        /// <param name="block">源 <see cref="ByteBlock"/>。</param>
        ...
        /// <summary>
        /// 隐式将 <see cref="ByteBlock"/> 转换为 <see cref="ReadOnlySequence{byte}"/>（使用 <see cref="UnreadMemory"/> 构造单段序列）。
        /// </summary>
        /// <param name="block">源 <see cref="ByteBlock"/>。</param>
