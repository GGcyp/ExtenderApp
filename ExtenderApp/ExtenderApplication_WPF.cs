...
{
    /// <summary>
    /// WPF 应用程序入口类（使用 Microsoft.Extensions.DependencyInjection + Autofac）。 负责：
    ...
    {
        /// <summary>
        /// 根服务提供者（由 Autofac 封装的 IServiceProvider）。 在应用退出时如果实现了 IDisposable 将被释放。
        /// </summary>
        ...
        /// <summary>
        /// 将指定文件夹（及其子目录）下的所有 .dll 加载到给定的 AssemblyLoadContext。 跳过空或无效的路径参数。
        /// </summary>
        ...
        /// <summary>
        /// 对外的错误记录入口（方法名保留原拼写 LogEorrer）。 使用注入的 ILogger 记录未处理异常信息（若 _logger 为 null 则静默忽略）。
        /// </summary>
