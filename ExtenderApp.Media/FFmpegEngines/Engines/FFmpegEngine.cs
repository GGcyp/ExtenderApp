...
namespace ExtenderApp.FFmpegEngines
{
    ...
    public unsafe partial class FFmpegEngine
    {
        ...
        /// <summary>
        /// FFmpeg 图像处理相关常量定义。
        /// 包含音视频重采样、像素格式转换、色彩空间、缩放算法等常用参数，便于调用
        /// FFmpeg swscale/swr 等接口时使用。 常量值与 FFmpeg
        /// C API 保持一致，适用于
        /// sws_getContext、swr_alloc_set_opts 等场景。
        /// </summary>
        /// <remarks>
        /// 例如：SWS_FAST_BILINEAR
        /// 表示快速双线性缩放，SWS_BICUBIC
        /// 表示双三次缩放，SWS_CS_ITU709 表示 BT.709 色彩空间等。
        /// </remarks>
        ...
        /// <summary>
        /// 创建并分配一个 RGB 图像缓冲区，并将其与指定 AVFrame
        /// 进行数据指针和行对齐绑定。 用于将托管字节数组与 FFmpeg 的
        /// AVFrame 结构关联，便于后续像素数据填充和渲染。
        /// 内部会从共享内存池租用字节数组，调用 CreateRGBBuffer 方法完成映射。
        /// </summary>
        /// <param name="rgbFrame">
        /// 目标 AVFrame 指针，写入 data 和 linesize 信息。
        /// </param>
        /// <param name="rgbBufferLength">
        /// RGB 缓冲区长度（字节），需根据像素格式和分辨率预先计算。
        /// </param>
        /// <param name="pixelFormat">
        /// 像素格式（AVPixelFormat），如 AV_PIX_FMT_RGB24。
        /// </param>
        ...
        /// <summary>
        /// 创建并填充 RGB 图像缓冲区，并将其数据指针和行对齐信息写入
        /// AVFrame。 用于将托管字节数组 rgbBuffer 映射到
        /// FFmpeg 的 AVFrame 结构，便于后续像素数据处理或渲染。
        /// 内部调用 FFmpeg 的 av_image_fill_arrays
        ...
        /// 用于根据上下文和解码设置，自动初始化视频和音频解码器，并管理解码过程中的资源。 支持自定义取消令牌和解码参数，便于多线程解码和参数调整。
        /// </summary>
        ...
        /// <summary>
        /// 根据指定的声道布局掩码（如
        /// AV_CH_LAYOUT_STEREO），创建并分配一个
        /// AVChannelLayout 结构体指针。
        /// 可用于音频重采样、格式转换等场景，便于配置 SwrContext
        /// 的输入/输出声道布局参数。 内部调用 FFmpeg 的
        /// av_channel_layout_default 方法进行初始化， 返回的
        /// NativeIntPtr&lt;AVChannelLayout&gt;
        /// 可直接用于 FFmpeg 相关接口。
        /// </summary>
        /// <param name="layout">
        /// 声道布局掩码（如
        /// AV_CH_LAYOUT_STEREO、AV_CH_LAYOUT_MONO 等）。
        /// </param>
        /// <returns>
        /// 分配并初始化好的 AVChannelLayout 指针封装。
        /// </returns>
        ...
        /// <summary>
        /// 跳转到指定时间戳（毫秒）的位置。 根据给定的 AVStream
        /// 指针，自动获取流索引和时间基准，调用重载方法完成跳转。 适用于需要按流类型（如视频或音频）精确定位的场景。
        /// </summary>
        ...
        /// 如果context或packet为空，则抛出此异常。
        /// </exception>
        /// <exception cref="FFmpegException">
        /// 如果从解码器接收数据包失败，则抛出此异常。
        /// </exception>
        ...
        /// <summary>
        /// 为指定的 AVFrame 分配音频或视频缓冲区。 此方法会根据帧的
        /// format、width/height（视频）或
        /// nb_samples/ch_layout（音频）等参数，
        /// 自动分配底层数据缓冲区，并填充 AVFrame 的
        /// data/linesize 指针，确保后续解码、重采样或渲染操作可用。
        /// 通常在设置好帧参数后调用本方法，避免 "Output changed" 等
        /// FFmpeg 错误。
        /// </summary>
        /// <param name="frame">
        /// 需要分配缓冲区的 AVFrame 指针。
        ...
        /// 便于后续音频重采样和格式转换，避免参数不匹配导致的 FFmpeg 错误。 通常在分配缓冲区前调用本方法。
        /// </summary>
        ...
        /// 包括采样格式、采样率、声道布局和采样数，确保后续重采样转换时参数匹配，避免
        /// FFmpeg "Output changed" 错误。 通常在调用
        /// GetBuffer 分配缓冲区前设置，适用于音频重采样和格式转换场景。
        /// </summary>
        /// <param name="frame">待设置参数的音频帧指针（AVFrame）。</param>
        /// <param name="dContext">音频解码器上下文，提供输入流参数。</param>
        /// <param name="sampleFormat">
        /// 目标采样格式（如 AV_SAMPLE_FMT_S16）。
        /// </param>
        /// <param name="sampleRate">
        /// 目标采样率（如 44100）。
        /// </param>
        /// <param name="channelLayout">
        /// 目标声道布局掩码（如 AV_CH_LAYOUT_STEREO）。
        /// </param>
        /// <returns>返回 0 表示设置成功。</returns>
        ...
        /// <summary> 将 FFmpeg
        /// 解码得到的音频帧数据（AVFrame）拷贝到托管字节数组，并返回实际数据长度。
        /// 该方法用于将 native PCM 数据从 FFmpeg 的 AVFrame
        /// 结构体中提取出来，便于后续播放或处理。 参数 targetChannel
        /// 通常为声道数（如 1=单声道，2=立体声），dataIndex 表示
        ...
        /// <summary>
        /// 从帧队列中获取一个 AVFrame 实例，如果队列为空则自动创建新的
        /// AVFrame。
        /// 用于解码、渲染等场景下的帧复用，减少频繁分配和释放内存，提高性能。 推荐在解码循环或帧处理流程中调用，避免重复分配帧资源。
        /// </summary>
        ...
        /// 会先清空帧内容（调用 FFmpeg 的
        /// av_frame_unref），再入队，避免内存泄漏和脏数据。 推荐在帧处理完毕后调用，便于资源管理和性能优化。
        /// </summary>
        /// <param name="frame">
        /// 需要被回收的 AVFrame 指针封装。
        /// </param>
        ...
        /// <summary>
        /// 使用 FFmpeg sws_scale
        /// 对视频帧进行像素格式转换和缩放处理（全帧转换）。 按照 FFmpegInfo
        /// 提供的高度，将 srcFrame 的全部像素数据转换到 dstFrame。 常用于解码后的视频渲染、帧导出或格式变换等场景。
        /// </summary>
        /// <param name="swsContext">
        /// 像素格式转换上下文（SwsContext），由
        /// CreateSwsContext 创建。
        /// </param>
        ...
        /// srcSliceY 或 srcSliceH 参数超出有效范围时抛出异常。
        /// </exception>
        ...
        /// 在设置好输入/输出参数后，必须调用本方法以完成 SwrContext
        /// 的初始化， 使其可以用于音频格式转换和重采样操作。 内部调用 FFmpeg
        /// 的 swr_init 方法，若初始化失败则抛出异常。
        /// </summary>
        ...
        /// 根据解码器上下文和解码器设置，配置 SwrContext
        /// 的输入输出参数（声道布局、采样格式、采样率）， 并调用
        ...
        /// 配置 SwrContext 以支持多声道和多格式音频转换。 内部自动分配输出声道布局指针，异常时自动释放资源。
        /// </summary>
        /// <param name="swrContext">音频重采样上下文指针。</param>
        /// <param name="outChannelLayout">
        /// 输出声道布局掩码（如 AV_CH_LAYOUT_STEREO）。
        /// </param>
        ...
        /// 直接指定输出和输入的 AVChannelLayout
        /// 指针、采样格式和采样率，配置 SwrContext
        /// 以支持多声道和多格式音频转换。 内部调用 FFmpeg 的
        ...
        /// <summary>
        /// 使用 FFmpeg 的 swr_convert_frame
        /// 方法进行音频重采样转换。 将输入 AVFrame
        /// 的音频数据转换为目标格式并写入输出 AVFrame。
        /// 适用于多声道、多采样率和多格式的音频转换场景。 调用前需确保
        /// SwrContext 已正确初始化，且 inFrame/outFrame
        /// 参数均为有效音频帧。 转换失败时会抛出异常并显示详细错误信息。
        /// </summary>
        /// <param name="swrContext">
        /// 音频重采样上下文指针（SwrContext），由 FFmpegEngine 创建和初始化。
        /// </param>
        ...
        /// <exception cref="ArgumentNullException">
        /// 当 swrContext、inFrame 或 outFrame 为空时抛出。
        /// </exception>
        /// <exception cref="FFmpegException">
        /// 转换失败时抛出，错误信息由 FFmpeg 返回。
        /// </exception>
        ...
        /// <summary>
        /// 按指定采样率对音频帧样本数进行重采样比例换算。 使用 FFmpeg 的
        /// av_rescale_rnd 方法，将 frame 的 nb_samples
        ...
        /// <summary>
        /// 将源 AVChannelLayout 结构体的数据复制到目标
        /// AVChannelLayout 结构体。
        /// 用于在音频处理、重采样等场景下，安全地复制声道布局信息，
        /// 保证目标布局与源布局一致。内部调用 FFmpeg 的
        /// av_channel_layout_copy 方法。
        /// </summary>
        ...
        /// <summary>
        /// 释放 FFmpegContext
        /// 相关资源，包括格式上下文、选项字典和解码器上下文集合。 用于确保所有底层指针和资源被正确释放，防止内存泄漏。
        /// </summary>
        /// <param name="context">
        /// 待释放的 FFmpegContext 实例。
        /// </param>
        ...
        /// <summary>
        /// 释放 FFmpegDecoderContext
        /// 相关资源，包括解码器上下文、参数和流指针。 用于安全释放解码器相关的底层资源。
        /// </summary>
        /// <param name="decoder">
        /// 待释放的 FFmpegDecoderContext 实例。
        /// </param>
        ...
        /// <summary>
        /// 释放 SwsContext（图像像素格式转换上下文）资源。 用于释放由
        /// FFmpeg sws_getContext 创建的像素格式转换上下文，防止内存泄漏。
        /// </summary>
        /// <param name="ptr">
        /// 待释放的 SwsContext 指针封装。
        /// </param>
        ...
        /// <summary>
        /// 释放 SwsFilter（图像滤镜）资源。 用于释放由 FFmpeg
        /// sws_getContext 创建的滤镜资源，防止内存泄漏。
        /// </summary>
        /// <param name="ptr">
        /// 待释放的 SwsFilter 指针封装。
        /// </param>
        ...
        /// <summary>
        /// 释放任意类型的非托管指针资源。 用于安全释放通过 FFmpeg
        /// 分配的结构体或缓冲区（如 AVChannelLayout、AVFrame
        /// 等），防止内存泄漏。 内部调用 FFmpeg 的 av_freep
        /// 方法释放底层内存，并移除指针引用。 推荐在不再需要相关资源时调用，确保及时回收内存。
        /// </summary>
        /// <typeparam name="T">
        /// 非托管类型（如 AVChannelLayout、AVFrame 等）。
        /// </typeparam>
        ...
        /// 用于重置或释放声道布局相关的内部数据，防止脏数据影响后续音频处理。 内部调用
        /// FFmpeg 的 av_channel_layout_uninit 方法，
        /// 仅清空结构体内容，不释放指针本身的内存。 推荐在复用或回收
        /// AVChannelLayout 结构体前调用。
        /// </summary>
        /// <param name="ptr">
        /// 待清空的 AVChannelLayout 指针封装。
        /// </param>
        ...
        /// </summary>
        /// <param name="pixelFormat">
        /// 像素格式（AVPixelFormat），如 YUV420P、RGB24 等。
        /// </param>
        ...
        /// 该方法通过 FFmpeg 的
        /// av_samples_get_buffer_size 计算实际音频数据长度，
        ...
        /// <summary>
        /// 获取指定帧的时间戳（毫秒）。
        /// 此方法会根据解码器上下文中的流信息，优先使用帧的
        /// PTS（显示时间戳），如果无效则使用 DTS（解码时间戳）。
        ...
        /// <summary>
        /// 获取指定帧的时间戳（毫秒）。 优先使用 PTS（显示时间戳），如果无效则使用
        /// DTS（解码时间戳）。 时间戳会根据流的 time_base 转换为毫秒，便于同步和显示。
        /// </summary>
        /// <param name="framePtr">AVFrame 指针封装，包含帧的原始数据。</param>
        /// <param name="streamPtr">
        /// AVStream 指针封装，包含流的时间基准。
        /// </param>
        /// <returns>当前帧的时间戳（单位：毫秒）。</returns>
        /// <exception cref="ArgumentNullException">
        /// framePtr 或 streamPtr 为空时抛出。
        /// </exception>
        ...
        /// 获取指定帧的时间戳（毫秒）。 优先使用 PTS（显示时间戳），如果无效则使用
        /// DTS（解码时间戳）。 时间戳会根据传入的
        /// time_base（流的时间基准）转换为毫秒，便于音视频同步和显示。
        /// 注意：视频流和音频流的 time_base 可以不同，需根据各自流的
        ...
        /// 持续时间原始值为流的时间基（time_base）单位，需结合流的时间基进行换算。 推荐使用本方法获取标准时间单位（毫秒），便于音视频同步和显示。
        /// </summary>
        ...
        /// <summary>
        /// 获取指定 AVFrame
        /// 的持续时间，并将其从流的时间基（time_base）单位转换为毫秒。
