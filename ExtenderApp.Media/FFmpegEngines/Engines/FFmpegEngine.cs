...
namespace ExtenderApp.Media.FFmpegEngines
{
    ...
    public unsafe partial class FFmpegEngine
    {
        ...
        /// <summary>
        /// FFmpeg 图像处理相关常量定义。
        /// 包含音视频重采样、像素格式转换、色彩空间、缩放算法等常用参数，便于调用 FFmpeg swscale/swr 等接口时使用。
        /// 常量值与 FFmpeg C API 保持一致，适用于 sws_getContext、swr_alloc_set_opts 等场景。
        /// </summary>
        /// <remarks>
        /// 例如：SWS_FAST_BILINEAR 表示快速双线性缩放，SWS_BICUBIC 表示双三次缩放，SWS_CS_ITU709 表示 BT.709 色彩空间等。
        /// </remarks>
        public const int SWR_FLAG_RESAMPLE = 0x1;
        /// <summary>SWS_ACCURATE_RND = 0x40000，精确舍入算法</summary>
        public const int SWS_ACCURATE_RND = 0x40000;
        /// <summary>SWS_AREA = 0x20，面积采样算法</summary>
        public const int SWS_AREA = 0x20;
        /// <summary>SWS_BICUBIC = 0x4，双三次插值算法</summary>
        public const int SWS_BICUBIC = 0x4;
        /// <summary>SWS_BICUBLIN = 0x40，线性双三次插值</summary>
        public const int SWS_BICUBLIN = 0x40;
        /// <summary>SWS_BILINEAR = 0x2，双线性插值算法</summary>
        public const int SWS_BILINEAR = 0x2;
        /// <summary>SWS_BITEXACT = 0x80000，位精确处理</summary>
        public const int SWS_BITEXACT = 0x80000;
        /// <summary>SWS_CS_BT2020 = 0x9，BT.2020 色彩空间</summary>
        public const int SWS_CS_BT2020 = 0x9;
        /// <summary>SWS_CS_DEFAULT = 0x5，默认色彩空间</summary>
        public const int SWS_CS_DEFAULT = 0x5;
        /// <summary>SWS_CS_FCC = 0x4，FCC 色彩空间</summary>
        public const int SWS_CS_FCC = 0x4;
        /// <summary>SWS_CS_ITU601 = 0x5，ITU-R BT.601 色彩空间</summary>
        public const int SWS_CS_ITU601 = 0x5;
        /// <summary>SWS_CS_ITU624 = 0x5，ITU-R BT.624 色彩空间</summary>
        public const int SWS_CS_ITU624 = 0x5;
        /// <summary>SWS_CS_ITU709 = 0x1，ITU-R BT.709 色彩空间</summary>
        public const int SWS_CS_ITU709 = 0x1;
        /// <summary>SWS_CS_SMPTE170M = 0x5，SMPTE 170M 色彩空间</summary>
        public const int SWS_CS_SMPTE170M = 0x5;
        /// <summary>SWS_CS_SMPTE240M = 0x7，SMPTE 240M 色彩空间</summary>
        public const int SWS_CS_SMPTE240M = 0x7;
        /// <summary>SWS_DIRECT_BGR = 0x8000，直接 BGR 排布</summary>
        public const int SWS_DIRECT_BGR = 0x8000;
        /// <summary>SWS_ERROR_DIFFUSION = 0x800000，误差扩散算法</summary>
        public const int SWS_ERROR_DIFFUSION = 0x800000;
        /// <summary>SWS_FAST_BILINEAR = 0x1，快速双线性插值</summary>
        public const int SWS_FAST_BILINEAR = 0x1;
        /// <summary>SWS_FULL_CHR_H_INP = 0x4000，完整色度采样输入</summary>
        public const int SWS_FULL_CHR_H_INP = 0x4000;
        /// <summary>SWS_FULL_CHR_H_INT = 0x2000，完整色度采样内部处理</summary>
        public const int SWS_FULL_CHR_H_INT = 0x2000;
        /// <summary>SWS_GAUSS = 0x80，高斯滤波算法</summary>
        public const int SWS_GAUSS = 0x80;
        /// <summary>SWS_LANCZOS = 0x200，Lanczos 插值算法</summary>
        public const int SWS_LANCZOS = 0x200;
        /// <summary>SWS_MAX_REDUCE_CUTOFF = 0.002D，最大降采样截止频率</summary>
        public const double SWS_MAX_REDUCE_CUTOFF = 0.002D;
        /// <summary>SWS_PARAM_DEFAULT = 0x1e240，默认参数</summary>
        public const int SWS_PARAM_DEFAULT = 0x1e240;
        /// <summary>SWS_POINT = 0x10，点采样算法</summary>
        public const int SWS_POINT = 0x10;
        /// <summary>SWS_PRINT_INFO = 0x1000，打印信息标志</summary>
        public const int SWS_PRINT_INFO = 0x1000;
        /// <summary>SWS_SINC = 0x100，Sinc 插值算法</summary>
        public const int SWS_SINC = 0x100;
        /// <summary>SWS_SPLINE = 0x400，样条插值算法</summary>
        public const int SWS_SPLINE = 0x400;
        /// <summary>SWS_SRC_V_CHR_DROP_MASK = 0x30000，源垂直色度丢弃掩码</summary>
        public const int SWS_SRC_V_CHR_DROP_MASK = 0x30000;
        /// <summary>SWS_SRC_V_CHR_DROP_SHIFT = 0x10，源垂直色度丢弃位移</summary>
        public const int SWS_SRC_V_CHR_DROP_SHIFT = 0x10;
        /// <summary>SWS_X = 0x8，未知/扩展标志</summary>
        ...
        /// 建议根据实际业务场景和解码速率调整，默认值为 300。
        /// </summary>
        private int MaxCacheFrameCount = 50;
        ...
        /// <summary>
        /// 创建图像像素格式转换上下文（SwsContext）。
        /// 用于将源图像从指定分辨率和像素格式转换为目标分辨率和像素格式，常用于视频解码后进行缩放或格式转换。
        /// </summary>
        ...
        /// <summary>
        /// 创建带有自定义滤镜的图像像素格式转换上下文（SwsContext）。
        /// 可指定源和目标滤镜，用于更复杂的图像处理场景（如锐化、模糊等）。
        /// </summary>
        /// <param name="srcW">源图像宽度（像素）。</param>
        /// <param name="srcH">源图像高度（像素）。</param>
        /// <param name="srcFormat">源像素格式（AVPixelFormat）。</param>
        ...
        /// <summary>
        /// 创建并分配一个 RGB 图像缓冲区，并将其与指定 AVFrame 进行数据指针和行对齐绑定。
        /// 用于将托管字节数组与 FFmpeg 的 AVFrame 结构关联，便于后续像素数据填充和渲染。
        /// 内部会从共享内存池租用字节数组，调用 CreateRGBBuffer 方法完成映射。
        /// </summary>
        ...
        /// <summary>
        /// 创建并填充 RGB 图像缓冲区，并将其数据指针和行对齐信息写入 AVFrame。
        /// 用于将托管字节数组 rgbBuffer 映射到 FFmpeg 的 AVFrame 结构，便于后续像素数据处理或渲染。
        /// 内部调用 FFmpeg 的 av_image_fill_arrays 方法，自动计算 data 和 linesize。
        /// </summary>
        ...
        /// <summary>
        /// 创建解码器控制器（FFmpegDecoderController）。
        /// 用于管理解码流程，包括解码启动、停止和取消等操作。
        /// 该方法会根据传入的上下文和解码设置，自动创建解码器集合和取消令牌源，并封装为控制器对象。
        /// </summary>
        ...
        public FFmpegDecoderController CreateDecoderController(FFmpegContext context, FFmpegDecoderSettings settings, CancellationTokenSource? source = null)
        {
            ...
            var collection = CreateDecoderCollection(context, source, settings);

            return new FFmpegDecoderController(this, context, collection, source);
        }

        /// <summary>
        /// 创建解码器集合（FFmpegDecoderCollection）。
        /// 用于根据上下文和解码设置，自动初始化视频和音频解码器，并管理解码过程中的资源。
        /// 支持自定义取消令牌和解码参数，便于多线程解码和参数调整。
        /// </summary>
        /// <param name="context">FFmpeg上下文，包含解码器集合和媒体信息。</param>
        /// <param name="source">可选的取消令牌源，若为空则自动创建。</param>
        /// <param name="settings">可选的解码器设置，若为空则使用默认设置。</param>
        /// <returns>FFmpegDecoderCollection 实例。</returns>
        public FFmpegDecoderCollection CreateDecoderCollection(FFmpegContext context, CancellationTokenSource? source = null, FFmpegDecoderSettings? settings = null)
        {
            if (context.IsEmpty)
            {
                throw new ArgumentNullException(nameof(context));
            ...
            return new FFmpegDecoderCollection(this, context.ContextCollection, context.Info, source.Token, settings);
        }

        /// <summary>
        /// 根据指定的声道布局掩码（如 AV_CH_LAYOUT_STEREO），创建并分配一个 AVChannelLayout 结构体指针。
        /// 可用于音频重采样、格式转换等场景，便于配置 SwrContext 的输入/输出声道布局参数。
        /// 内部调用 FFmpeg 的 av_channel_layout_default 方法进行初始化，
        /// 返回的 NativeIntPtr&lt;AVChannelLayout&gt; 可直接用于 FFmpeg 相关接口。
        /// </summary>
        ...
        /// <summary>
        /// 向解码器发送一个数据包（AVPacket）。
        /// 用于将编码后的数据（如音视频流中的一帧）传递给解码器进行解码处理。
        /// </summary>
        ...
        /// <summary>
        /// 从解码器接收一个数据包（AVPacket）。
        /// 用于从解码器获取编码后的数据（如转码或编码场景）。
        /// </summary>
        /// <param name="codecContext">解码器上下文指针。</param>
        ...
        public void ReturnPacket(ref NativeIntPtr<AVPacket> packet)
        {
            ...
            else if (_packetQueue.Count >= MaxCacheFrameCount)
            {
                Free(ref packet);
                return;
            }
        ...
        /// <summary>
        /// 向解码器发送一个帧（AVFrame）。
        /// 用于将解码后的原始帧数据（如音视频帧）传递给编码器或滤镜处理。
        /// </summary>
        /// <param name="codecContext">解码器上下文指针。</param>
        ...
        /// <summary>
        /// 为指定的 AVFrame 分配音频或视频缓冲区。
        /// 此方法会根据帧的 format、width/height（视频）或 nb_samples/ch_layout（音频）等参数，
        /// 自动分配底层数据缓冲区，并填充 AVFrame 的 data/linesize 指针，确保后续解码、重采样或渲染操作可用。
        /// 通常在设置好帧参数后调用本方法，避免 "Output changed" 等 FFmpeg 错误。
        /// </summary>
        ...
        /// 包括采样格式、采样率、声道布局等，确保输出帧与重采样上下文（SwrContext）参数一致，
        /// 便于后续音频重采样和格式转换，避免参数不匹配导致的 FFmpeg 错误。
        /// 通常在分配缓冲区前调用本方法。
        /// </summary>
        ...
        {
            return SettingsAudioFrame(frame, dContext, settings.SampleFormat, settings.SampleRate, settings.ChannelLayout);
        }

        /// <summary>
        /// 设置音频输出帧（AVFrame）的参数，使其与指定的解码器上下文和目标采样参数一致。
        /// 包括采样格式、采样率、声道布局和采样数，确保后续重采样转换时参数匹配，避免 FFmpeg "Output changed" 错误。
        /// 通常在调用 GetBuffer 分配缓冲区前设置，适用于音频重采样和格式转换场景。
        /// </summary>
        /// <param name="frame">待设置参数的音频帧指针（AVFrame）。</param>
        /// <param name="dContext">音频解码器上下文，提供输入流参数。</param>
        ...
        /// <summary>
        /// 将 FFmpeg 解码得到的音频帧数据（AVFrame）拷贝到托管字节数组，并返回实际数据长度。
        /// 该方法用于将 native PCM 数据从 FFmpeg 的 AVFrame 结构体中提取出来，便于后续播放或处理。
        /// 参数 targetChannel 通常为声道数（如 1=单声道，2=立体声），dataIndex 表示 FFmpeg 帧的 data 数组下标（一般为 0）。
        /// 注意：返回的字节数组来自共享内存池，使用完毕后应归还（ArrayPool<byte>.Shared.Return）。
        /// </summary>
        /// <param name="frame">音频帧指针（NativeIntPtr&lt;AVFrame&gt;），包含 PCM 数据。</param>
        /// <param name="targetChannel">目标声道数（如 1=单声道，2=立体声）。</param>
        /// <param name="length">输出参数，返回实际拷贝的数据长度（字节）。</param>
        /// <param name="dataIndex">FFmpeg 帧的 data 数组下标，默认 0。</param>
        /// <returns>包含 PCM 音频数据的字节数组。</returns>
        ...
        /// <summary>
        /// 从帧队列中获取一个 AVFrame 实例，如果队列为空则自动创建新的 AVFrame。
        /// 用于解码、渲染等场景下的帧复用，减少频繁分配和释放内存，提高性能。
        /// 推荐在解码循环或帧处理流程中调用，避免重复分配帧资源。
        /// </summary>
        ...
        /// <summary>
        /// 回收一个 AVFrame 实例，将其放回帧队列以供后续复用。
        /// 会先清空帧内容（调用 FFmpeg 的 av_frame_unref），再入队，避免内存泄漏和脏数据。
        /// 推荐在帧处理完毕后调用，便于资源管理和性能优化。
        /// </summary>
        ...
        /// <summary>
        /// 使用 FFmpeg sws_scale 对视频帧进行像素格式转换和缩放处理（全帧转换）。
        /// 按照 FFmpegInfo 提供的高度，将 srcFrame 的全部像素数据转换到 dstFrame。
        /// 常用于解码后的视频渲染、帧导出或格式变换等场景。
        /// </summary>
        ...
        /// 可指定源帧的起始行（srcSliceY）和转换高度（srcSliceH），将部分像素数据转换到目标帧。
        /// 适用于分片渲染、并行处理或自定义区域转换等高级场景。
        /// </summary>
        /// <param name="swsContext">像素格式转换上下文（SwsContext），由 CreateSwsContext 创建。</param>
        /// <param name="srcFrame">源帧指针，包含原始数据和行对齐信息。</param>
        ...
        /// <summary>
        /// 初始化音频重采样上下文（SwrContext）。
        /// 在设置好输入/输出参数后，必须调用本方法以完成 SwrContext 的初始化，
        /// 使其可以用于音频格式转换和重采样操作。
        ...
        /// <summary>
        /// 设置并初始化音频重采样上下文（SwrContext）。
        /// 根据解码器上下文和解码器设置，配置 SwrContext 的输入输出参数（声道布局、采样格式、采样率），
        /// 并调用 SwrContextInit 完成初始化，使其可用于音频重采样和格式转换。
        /// </summary>
        /// <param name="swrContext">音频重采样上下文指针。</param>
        ...
        public void SetSwrContextOptionsAndInit(NativeIntPtr<SwrContext> swrContext, FFmpegDecoderContext dContext, FFmpegDecoderSettings settings)
        {
            ...
            SetSwrContextOptions(
                swrContext,
                settings.ChannelLayout,                  // 输出声道布局
                (AVSampleFormat)settings.SampleFormat,   // 输出采样格式
        ...
        /// <summary>
        /// 设置音频重采样上下文（SwrContext）的输入输出参数（支持声道布局掩码）。
        /// 根据指定的输出声道布局掩码、采样格式、采样率，以及输入声道布局、采样格式、采样率，
        /// 配置 SwrContext 以支持多声道和多格式音频转换。
        /// 内部自动分配输出声道布局指针，异常时自动释放资源。
        /// </summary>
        /// <param name="swrContext">音频重采样上下文指针。</param>
        ...
        /// <summary>
        /// 设置音频重采样上下文（SwrContext）的输入输出参数（支持声道布局指针）。
        /// 直接指定输出和输入的 AVChannelLayout 指针、采样格式和采样率，配置 SwrContext 以支持多声道和多格式音频转换。
        /// 内部调用 FFmpeg 的 swr_alloc_set_opts2 方法完成参数设置。
        /// </summary>
        /// <param name="swrContext">音频重采样上下文指针。</param>
        ...
        /// <summary>
        /// 使用 FFmpeg 的 swr_convert_frame 方法进行音频重采样转换。
        /// 将输入 AVFrame 的音频数据转换为目标格式并写入输出 AVFrame。
        /// 适用于多声道、多采样率和多格式的音频转换场景。
        /// 调用前需确保 SwrContext 已正确初始化，且 inFrame/outFrame 参数均为有效音频帧。
        /// 转换失败时会抛出异常并显示详细错误信息。
        /// </summary>
        ...
        /// <summary>
        /// 按指定采样率对音频帧样本数进行重采样比例换算。
        /// 使用 FFmpeg 的 av_rescale_rnd 方法，将 frame 的 nb_samples 按目标采样率（targetSampleRate）与当前解码器采样率进行比例缩放，
        /// 并可指定舍入方式（rounding），常用于音频重采样、时间戳换算等场景。
        /// </summary>
        ...
        /// <summary>
        /// 将源 AVChannelLayout 结构体的数据复制到目标 AVChannelLayout 结构体。
        /// 用于在音频处理、重采样等场景下，安全地复制声道布局信息，
        ...
        {
            return ffmpeg.av_get_bytes_per_sample(format);
        }
        ...
        /// <summary>
        /// 释放 FFmpegContext 相关资源，包括格式上下文、选项字典和解码器上下文集合。
        /// 用于确保所有底层指针和资源被正确释放，防止内存泄漏。
        /// </summary>
        ...
        /// <summary>
        /// 释放 FFmpegDecoderContext 相关资源，包括解码器上下文、参数和流指针。
        /// 用于安全释放解码器相关的底层资源。
        /// </summary>
        ...
        /// <summary>
        /// 释放 FFmpegDeciderContextCollection 集合中的所有解码器上下文资源。
        /// 会遍历集合并逐一释放每个解码器上下文。
        /// </summary>
        /// <param name="ptr">待释放的 FFmpegDeciderContextCollection 实例。</param>
        ...
        /// <summary>
        /// 释放 SwsContext（图像像素格式转换上下文）资源。
        /// 用于释放由 FFmpeg sws_getContext 创建的像素格式转换上下文，防止内存泄漏。
        /// </summary>
        ...
        /// <summary>
        /// 释放 SwsFilter（图像滤镜）资源。
        /// 用于释放由 FFmpeg sws_getContext 创建的滤镜资源，防止内存泄漏。
        /// </summary>
        ...
        /// <summary>
        /// 释放 AVFilterGraph（滤镜图）资源。
        /// 用于释放由 FFmpeg 创建的滤镜图结构，防止内存泄漏。
        /// </summary>
        ...
        /// <summary>
        /// 释放任意类型的非托管指针资源。
        /// 用于安全释放通过 FFmpeg 分配的结构体或缓冲区（如 AVChannelLayout、AVFrame 等），防止内存泄漏。
        /// 内部调用 FFmpeg 的 av_freep 方法释放底层内存，并移除指针引用。
        /// 推荐在不再需要相关资源时调用，确保及时回收内存。
        /// </summary>
        ...
        /// <summary>
        /// 刷新解码器上下文缓冲区，清空内部缓存的数据。
        /// 常用于解码器重置或切换流时，确保后续解码操作的正确性。
        /// </summary>
        ...
        /// <summary>
        /// 刷新格式上下文缓冲区，清空内部缓存的数据。
        /// 常用于格式上下文重置或切换流时，确保后续操作的正确性。
        /// </summary>
        ...
        /// <summary>
        /// 清空 AVChannelLayout 声道布局结构体的内容。
        /// 用于重置或释放声道布局相关的内部数据，防止脏数据影响后续音频处理。
        /// 内部调用 FFmpeg 的 av_channel_layout_uninit 方法，
        /// 仅清空结构体内容，不释放指针本身的内存。
        /// 推荐在复用或回收 AVChannelLayout 结构体前调用。
        /// </summary>
        ...
        /// <summary>
        /// 获取指定像素格式和媒体信息（FFmpegInfo）下的图像缓冲区所需字节数。
        /// 通常用于分配视频帧内存或判断缓冲区大小，底层调用 FFmpeg 的 av_image_get_buffer_size。
        /// </summary>
        ...
        /// <summary>
        /// 获取指定像素格式和帧（AVFrame）下的图像缓冲区所需字节数。
        /// 通常用于分配视频帧内存或判断缓冲区大小，底层调用 FFmpeg 的 av_image_get_buffer_size。
        /// </summary>
        /// <param name="pixelFormat">像素格式（AVPixelFormat），如 YUV420P、RGB24 等。</param>
        ...
        /// <summary>
        /// 获取指定像素格式、宽度、高度和对齐方式下的图像缓冲区所需字节数。
        /// 通常用于分配视频帧内存或判断缓冲区大小，底层调用 FFmpeg 的 av_image_get_buffer_size。
        /// </summary>
        /// <param name="pixelFormat">像素格式（AVPixelFormat），如 YUV420P、RGB24 等。</param>
        /// <param name="width">图像宽度（像素）。</param>
        ...
        /// <summary>
        /// 获取指定 AVFrame 音频帧的 PCM 数据缓冲区所需字节数。
        /// 该方法通过 FFmpeg 的 av_samples_get_buffer_size 计算实际音频数据长度，
        /// 适用于分配托管缓冲区或进行数据拷贝时的长度判断。
        /// 参数 align 通常为 1，表示按字节对齐。
        /// </summary>
