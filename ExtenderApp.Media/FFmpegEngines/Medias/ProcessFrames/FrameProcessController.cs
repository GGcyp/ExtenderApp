...
namespace ExtenderApp.FFmpegEngines.Medias
{
    ...
    /// <para>
    /// 本类位于“解码”与“渲染/播放”之间，承担“帧调度（Frame Scheduling）”职责：
    /// </para>
    /// <list type="bullet">
    /// <item>
    /// <description>
    /// 从每路 <see cref="IFFmpegDecoder"/> 的输出队列中窥视（peek）队首帧，判断是否应当输出/丢弃/等待。
    /// </description>
    /// </item>
    /// <item>
    /// <description>
    /// 在 Seek 或重建播放链路时，基于 <c>generation</c> 丢弃旧代帧，保证不会输出跳转前的缓存帧。
    /// </description>
    /// </item>
    /// <item>
    /// <description>
    /// 以音频作为主时钟（若存在音频解码器），否则退化使用视频作为主时钟；并在开始播放时对齐首帧时间线。
    /// </description>
    /// </item>
    /// <item>
    /// <description>
    /// 向上层返回“距离下一帧还有多久”的建议等待时间（毫秒），用于控制播放循环的 sleep/spin 策略。
    /// </description>
    /// </item>
    /// </list>
    /// <para>
    /// 资源管理：所有从解码器队列中出队的 <see cref="FFmpegFrame"/> 都必须调用 <see cref="FFmpegFrame.Dispose"/> 释放底层缓冲。
    /// </para>
    /// </summary>
    internal class FrameProcessController : DisposableObject, IFrameProcessController
    {
        ...
        /// 执行一次帧调度处理。
        /// <para>
        /// 会分别处理音频与视频两路队列，并返回“下一次应该等待多久”的建议值（毫秒）。
        /// </para>
        /// </summary>
        ...
        /// 距离下一帧到达的等待时间（毫秒）。
        /// <list type="bullet">
        /// <item><description>&gt; 0：存在未来帧（Pts &gt; position），应等待该差值。</description></item>
        /// <item><description>-1：暂无可用于计算等待的未来帧（队列为空或仅有过期帧）。</description></item>
        /// </list>
        ...
        /// 逻辑为：
        /// <list type="bullet">
        /// <item><description>peek 队首帧：若 generation 不匹配则出队并释放（旧代丢弃）。</description></item>
        /// <item><description>若 <c>Pts &gt; position</c>：返回差值作为建议等待时间。</description></item>
        /// <item><description>若 <c>Pts ≤ position</c>：尝试出队；若时间误差在阈值内则输出，否则丢弃。</description></item>
        /// </list>
        /// </para>
        /// <para>
        /// 资源约束：所有成功出队的 <see cref="FFmpegFrame"/> 必须调用 <see cref="FFmpegFrame.Dispose"/> 释放底层缓冲。
        /// </para>
        /// </summary>
