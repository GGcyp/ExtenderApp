...
namespace ExtenderApp.Service
{
    internal class NavigationService : INavigationService
    {
        ...
        public NavigationService(IServiceProvider serviceProvider, ILogService logService)
        {
        ...
        public IView NavigateTo(Type targetViewType, IView oldView)
        {
            ...
            if (newView is null)
            {
                _logService.Error(string.Format("无法切换到视图从{0}切换到视图{1}", oldView.ViewInfo.ViewName, targetViewType?.Name), nameof(INavigationService), new ArgumentNullException());
            ...
            _logService.Info(string.Format("成功切换到视图{0}", targetViewType.Name), nameof(INavigationService));
