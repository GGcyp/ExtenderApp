...
namespace ExtenderApp.Common.Networks
{
    ...
    internal class SocketTcpListenerLinker : TcpListenerLinker
    {
        ...
        /// <summary>
        /// 轻量级的异步“手动复位事件”（Async Manual TryRelease Event）。 用于实现“暂停/恢复”闸门：
        /// - Set()：打开闸门，允许等待者继续；
        /// - TryRelease()：关闭闸门，使后续等待者阻塞；
        /// - WaitAsync()：等待闸门被打开。
        /// </summary>
        /// <remarks>
        /// 设计要点：
        ...
        /// - Set 为幂等操作，重复调用不会抛异常；
        /// - TryRelease 仅在当前任务已完成时重建新的 <see cref="TaskCompletionSource{TResult}"/>，避免不必要的分配；
