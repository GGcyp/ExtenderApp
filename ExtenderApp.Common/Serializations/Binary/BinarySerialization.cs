...
namespace ExtenderApp.Common.Serializations.Binary
{
    ...
    internal class BinarySerialization : Serialization, IBinarySerialization
    {
        ...
        /// 建议的连续内存大小。
        /// </summary>
        /// <remarks>值为1MB（1024 * 1024字节）。</remarks>
        ...
        public override T? Deserialize<T>(ReadOnlySequence<byte> memories) where T : default
        {
            ...
            return _resolver.GetFormatterWithVerify<T>().Deserialize(ref buffer);
        }
        ...
        // switch (compression) { case CompressionType.Lz4Block: ToLz4Block(readOnlyMemories, ref buffer); break;
        ...
        // int sequenceCount = 0; int extHeaderSize = 0; foreach (var item in readOnlyMemories) { sequenceCount++; extHeaderSize +=
        // GetUInt32WriteSize((uint)item.Length); }

        // Serialize(new ExtensionHeader(Lz4bufferArray, (uint)extHeaderSize), out ByteBuffer tempBuffer); tempBuffer.TryCopyTo(ref buffer);
        // tempBuffer.Dispose(); _convert.WriteArrayHeader(ref buffer, sequenceCount + 1); foreach (var item in readOnlyMemories) {
        ...
        // // Try to find LZ4buffer var header = _extensionFormatter.Deserialize(ref inputbuffer); if (header.IsEmpty) { outbuffer.Dispose(); return
        // false; }

        // switch (header.TypeCode) { case 0: outbuffer.Dispose(); return false;

        // case Lz4Block: return FormLz4Block(ref inputbuffer, out outbuffer);

        // case Lz4bufferArray: return FormLz4bufferArray(ref inputbuffer, out outbuffer);
        ...
        // //这个整数表示数据在解压缩之后的长度。这样的设计允许接收方在解压数据之前就知道最终数据的大小， //这对于数据处理的效率和正确性至关重要。 int uncompressedLength = _intFormatter.Deserialize(ref peekbuffer);

        // //接下来开始解压 ReadOnlySequence<byte> compressedData = peekbuffer.Sequence.Slice(peekbuffer.Position);

        // Span<byte> uncompressedSpan = outBuffer.GetSpan(uncompressedLength).Slice(0, uncompressedLength); int actualUncompressedLength =
        // LZ4Operation(compressedData, uncompressedSpan, LZ4CodecDecode); outBuffer.WriteAdvance(actualUncompressedLength);
        ...
        // // 切换成原读取器 inputbuffer = peekbuffer;

        // // 开始读取 [Ext(98:int,int...), bin,bin,bin...] var sequenceCount = arrayLength - 1; var uncompressedLengths =
        // ArrayPool<int>.Shared.Rent(sequenceCount); try { for (int i = 0; i < sequenceCount; i++) { uncompressedLengths[i] =
        // _intFormatter.Deserialize(ref inputbuffer); }
