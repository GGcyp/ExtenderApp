...
namespace ExtenderApp.Common
{
    ...
    public static class SerializationExtensions
    {
        ...
        public static T? Deserialize<T>(this ISerialization serialization, ReadOnlySpan<byte> span, ICompression compression)
        {
            ...
            {
                var result = serialization.Deserialize<T>(block);
                block.TryRelease();
                return result;
            }
        ...
        /// <summary> 从由 <see cref="IFileOperateProvider"/> 根据 <see cref="FileOperateInfo"/> 创建的文件操作器中读取指定长度数据并反序列化为目标类型。 </summary> <typeparam
        /// name="T">目标对象类型。</typeparam> <param name="serialization">用于反序列化的 <see cref="ISerialization"/> 实例。</param> <param name="provider">用于获取 <see
        /// cref="IFileOperate"/> 的提供者。</param> <param name="info">用于创建文件操作器的文件操作配置信息。</param> <param name="position">读取的起始位置（字节偏移）。</param> <param
        /// name="length">要读取的字节数。</param> <returns>反序列化得到的对象实例；若数据为空或解析失败可返回 <c>null</c>（由具体实现决定）。returns> <exception cref="ArgumentNullException">当
        /// <paramref name="provider"/> 为 <c>null</c> 时抛出。</exception>
        ...
        public static T? Deserialize<T>(this ISerialization serialization, IFileOperate operate, long position = 0)
        {
            ...
            var res = operate.Read(position);
            if (!res.IsSuccess || res.Value == null)
                return default;
            return serialization.Deserialize<T>(res.Value.AsSpan());
        }
        ...
        public static T? Deserialize<T>(this ISerialization serialization, IFileOperate operate, long position, int length)
        {
            ...
            T? result = serialization.Deserialize<T>(buffer.CommittedSpan);
            buffer.TryRelease();
            return result;
        }
        ...
        public static T? Deserialize<T>(this ISerialization serialization, ICompression compression, IFileOperate operate, long position = 0)
        {
            ...
            int length = (int)(operate.Capacity - position);
            var buffer = MemoryBlock<byte>.GetBuffer(length);
            operate.Read(length, buffer, position);
            var result = serialization.Deserialize<T>(buffer, compression);
            buffer.TryRelease();
            return result;
        }
        ...
        public static T? Deserialize<T>(this ISerialization serialization, ICompression compression, IFileOperate operate, long position, int length)
        {
            ...
            var buffer = MemoryBlock<byte>.GetBuffer((int)(operate.Capacity - position));
            operate.Read(length, buffer, position);
            var result = serialization.Deserialize<T>(buffer, compression);
            buffer.TryRelease();
            return result;
