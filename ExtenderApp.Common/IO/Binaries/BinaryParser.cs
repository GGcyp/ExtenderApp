...
namespace ExtenderApp.Common.IO.Binary
{
    ...
    internal class BinaryParser : FileParser, IBinaryParser
    {
        ...
        private readonly ByteBlockConvert _convert;

        protected override string FileExtension { get; }

        public BinaryParser(IBinaryFormatterResolver binaryFormatterResolver, SequencePool<byte> sequencePool, ByteBlockConvert convert, BinaryOptions options, IFileOperateProvider provider) : base(provider)
        {
        ...
        public void Serialize<T>(T value, byte[] bytes, out long length)
        {
            ...
            var block = new ByteBlock(_pool);
            Serialize(ref block, value);
            block.TryCopyTo(bytes);
            block.Dispose();
        }
        ...
        public void Serialize<T>(ref ByteBlock block, T value)
        {
            try
            {
                _resolver.GetFormatterWithVerify<T>().Serialize(ref block, value);
            }
            catch (Exception ex)
            {
                throw;
        ...
        public void Serialize<T>(Stream stream, T value)
        {
            ...
            foreach (ReadOnlyMemory<byte> segment in block.Sequence)
            {
        ...
        public T? Deserialize<T>(ReadOnlyMemory<byte> memory)
        {
            ...
            ByteBlock block = new ByteBlock(memory);
            return Deserialize<T>(ref block);
        }
        ...
        /// <param name="rblock">用于读取数据的<see cref="ByteBlock"/>对象</param>
        /// <returns>反序列化后的对象，如果剩余数据为0且没有默认格式化器，则返回默认值</returns>
        private T? Deserialize<T>(ref ByteBlock rblock)
        {
            if (rblock.Remaining == 0)
            {
                var formatter = _resolver.GetFormatter<T>();
                return default;
            }

            if (TryDecompress(ref rblock, out var block))
            {
                rblock.Dispose();
                rblock = new ByteBlock(block.Sequence);
            }

            T? result = _resolver.GetFormatterWithVerify<T>().Deserialize(ref rblock);
            rblock.Dispose();
            return result;
        }
        ...
        public T? Deserialize<T>(Stream stream)
        {
            ...
            ByteBlock block = new ByteBlock(_pool);
            int bytesRead = 0;
            do
            {
                Span<byte> span = block.GetSpan(stream.CanSeek ? (int)System.Math.Min(SuggestedContiguousMemorySize, stream.Length - stream.Position) : 0);
                bytesRead = stream.Read(span);
                block.WriteAdvance(bytesRead);
            }
            while (bytesRead > 0);

            result = DeserializeFromSequenceAndRewindStreamIfPossible<T>(stream, block.Sequence);
        ...
        private T? DeserializeFromSequenceAndRewindStreamIfPossible<T>(Stream stream, ReadOnlySequence<byte> sequence)
        {
            ...
            ByteBlock block = new ByteBlock(sequence);

            T? result = Deserialize<T>(ref block);

            //读取完成后返回到未读取的第一个字节
            if (stream.CanSeek && !block.End)
            {
                long bytesNotRead = block.Sequence.Slice(block.Position).Length;
        ...
        private bool TryDeserializeFromMemoryStream<T>(Stream stream, out T? result)
        {
            if (stream is MemoryStream ms && ms.TryGetBuffer(out ArraySegment<byte> streamBuffer))
            {
                ...
                var block = new ByteBlock(buffer);

                result = Deserialize<T>(ref block);

                var bytesRead = buffer.Slice(0, (int)block.Consumed).Length;
        ...
        protected override void ExecuteWrite<T>(IFileOperate fileOperate, T value, long position)
        {
            ...
            fileOperate.Write(position, block);
            block.Dispose();
        }
        ...
        {
            var scratchWriter = new ByteBlock(_pool);
            Serialize(ref scratchWriter, value);

            var block = new ByteBlock(_pool);
            ToLz4(scratchWriter.Sequence, ref block, compression);

            byte[] result = block.ToArray();
        ...
        {
            ByteBlock block = new ByteBlock(_pool);
            ToLz4(readOnlyMemories, ref block, CompressionType.Lz4BlockArray);
            var result = block.ToArray();
            block.Dispose();
            return result;
        }
        ...
        public void ToLz4(in ReadOnlySequence<byte> readOnlyMemories, ref ByteBlock block, CompressionType compression)
        {
            if (readOnlyMemories.Length < CompressionMinLength || compression == CompressionType.None)
            {
                block.Write(readOnlyMemories);
                return;
            }
            ...
            switch (compression)
            {
                case CompressionType.Lz4Block:
                    ...
                    try
                    {
                        ...
                        _resolver.GetFormatterWithVerify<ExtensionHeader>().Serialize(ref block, new ExtensionHeader(Lz4Block, (uint)lz4Length));
                        _resolver.GetFormatterWithVerify<int>().Serialize(ref block, (int)readOnlyMemories.Length);
                        // 将LZ4压缩后的数据写入
                        block.Write(lz4Bytes.AsSpan(0, lz4Length));
                    }
                    finally
                    {
                ...
                case CompressionType.Lz4BlockArray:
                    ...
                    _convert.WriteArrayHeader(ref block, sequenceCount + 1);
                    Serialize(ref block, new ExtensionHeader(Lz4BlockArray, (uint)extHeaderSize));
                    IBinaryFormatter<int> intFormatter = _resolver.GetFormatterWithVerify<int>();
                    foreach (var item in readOnlyMemories)
                    {
                        intFormatter.Serialize(ref block, item.Length);
                    }

                    foreach (var item in readOnlyMemories)
                    {
                        maxCompressedLength = LZ4Codec.MaximumOutputLength(item.Length);
                        var lz4Span = block.GetSpan(maxCompressedLength + 5);
                        int lz4Length = LZ4Codec.Encode(item.Span, lz4Span.Slice(5, lz4Span.Length - 5));
                        WriteBin32Header((uint)lz4Length, lz4Span);
                        block.WriteAdvance(lz4Length + 5);
                    }
        ...
        private bool TryDecompress(ref ByteBlock inputBlock, out ByteBlock outBlock)
        {
            outBlock = new ByteBlock(_pool);
            if (inputBlock.End)
            {
                inputBlock.Dispose();
                return false;
            ...
            var header = _resolver.GetFormatterWithVerify<ExtensionHeader>().Deserialize(ref inputBlock);
            if (header.IsEmpty)
            {
                outBlock.Dispose();
                return false;
            ...
            {
                var extReader = inputBlock.CreatePeekBlock();
                ...
                Span<byte> uncompressedSpan = outBlock.GetSpan(uncompressedLength).Slice(0, uncompressedLength);
                int actualUncompressedLength = LZ4Operation(compressedData, uncompressedSpan, LZ4CodecDecode);
                outBlock.WriteAdvance(actualUncompressedLength);

                return true;
            }

            var peekReader = inputBlock.CreatePeekBlock();
            var arrayLength = _convert.ReadArrayHeader(ref peekReader);
            if (arrayLength == 0) return false;
            header = Deserialize<ExtensionHeader>(ref inputBlock);
            ...
            try
            {
                ...
                {
                    uncompressedLengths[i] = intFormatter.Deserialize(ref inputBlock);
                }

                for (int i = 0; i < sequenceCount; i++)
                {
                    var uncompressedLength = uncompressedLengths[i];
                    ReadOnlySequence<byte> lz4Block = _convert.ReadBytes(ref inputBlock);
                    Span<byte> uncompressedSpan = outBlock.GetSpan(uncompressedLength);
