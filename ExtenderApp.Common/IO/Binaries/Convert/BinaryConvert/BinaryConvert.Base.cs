...
{
    /// <summary>
    /// 提供基于 MessagePack 兼容的二进制读写/判别辅助能力的基础实现。
    /// </summary>
    /// <remarks>
    /// 通过 <see cref="BinaryOptions"/> 指定编码、范围、常量等运行时选项，
    /// 并提供对类型码的快速判别（fixint/fixmap/fixarray/fixstr 等）。
    /// </remarks>
    public partial class BinaryConvert
    {
        ...
        /// <summary>
        /// 提供 MessagePack 类型码常量（来自 <see cref="BinaryOptions"/>）。
        /// 仅为便捷访问器。
        /// </summary>
        public BinaryCode BinaryCode => BinaryOptions.BinaryCode;

        /// <summary>
        /// 提供固定段（fix）边界配置（来自 <see cref="BinaryOptions"/>）。
        /// 仅为便捷访问器。
        /// </summary>
        public BinaryRang BinaryRang => BinaryOptions.BinaryRang;

        /// <summary>
        /// 提供各数值类型总字节长度定义（来自 <see cref="BinaryOptions"/>）。
        /// 仅为便捷访问器。
        /// </summary>
        public BinaryLength BinaryLength => BinaryOptions.BinaryLength;

        /// <summary>
        /// 提供时间/日期的编码常量（来自 <see cref="BinaryOptions"/>）。
        /// 仅为便捷访问器。
        /// </summary>
        public BinaryDateTime BinaryDateTime => BinaryOptions.BinaryDateTime;

        /// <summary>
        /// 内部解码器与跳转表集合，按类型码分发读取逻辑。
        /// </summary>
        private BinaryConvertDecoders Decoders { get; }

        /// <summary>
        /// 获取用于字节与字符串互转的字符编码（通常为 UTF-8）。
        /// </summary>
        public Encoding BinaryEncoding => BinaryOptions.BinaryEncoding;

        /// <summary>
        /// 使用指定选项初始化 <see cref="BinaryConvert"/>。
        /// </summary>
        /// <param name="options">二进制选项（包含类型码、范围、长度与时间常量等）。</param>
        ...
        //    TypeLookupTable[Float32] = BinaryType.Float32;
        //    TypeLookupTable[Float64] = BinaryType.Float32;
