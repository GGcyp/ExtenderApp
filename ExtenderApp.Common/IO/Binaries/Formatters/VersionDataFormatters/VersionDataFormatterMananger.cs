...
namespace ExtenderApp.Common.IO.Binary.Formatters
{
    ...
    internal class VersionDataFormatterMananger<T> : ResolverFormatter<VersionData<T>>, IVersionDataFormatterMananger<T>
    {
        ...
        public override VersionData<T> Deserialize(ref ByteBlock block)
        {
        ...
        public override void Serialize(ref ByteBlock block, VersionData<T> value)
        {
            if (Formatters.Count == 0)
            {
        ...
        public void Serialize(ref ByteBlock block, T value, Version version)
        {
            if (Formatters.Count == 0)
            {
                throw new InvalidOperationException($"没有可用的格式化器，请先添加格式化器。 {typeof(T).FullName}");
        ...
        public T Deserialize(ref ByteBlock block, Version version)
        {
            if (version == null)
            {
        ...
        public void Serialize(ref ByteBlock writer, T value)
        {
        ...
        T IBinaryFormatter<T>.Deserialize(ref ByteBlock block)
        {
            var version = _version.Deserialize(ref block);
