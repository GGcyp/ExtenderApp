...
namespace ExtenderApp.Common.IO.Binary.Formatters
{
    ...
    /// 封装共享依赖（<see cref="ByteBlockConvert"/> 与 <see cref="BinaryOptions"/>），
    /// 具体读写逻辑由更具体的派生类实现。
    /// </summary>
    ...
    public abstract class BinaryFormatter<T> : BaseBinaryFormatter<T>
    {
        ...
        /// <summary>
        /// 面向 <see cref="ByteBlock"/> 的写入适配器集合，用于输出各原始类型/头部等。
        /// </summary>
        protected readonly ByteBlockConvert _blockConvert;
        ...
        protected BinaryFormatter(ByteBlockConvert blockConvert, BinaryOptions options)
        {
            _binaryOptions = options;
            _blockConvert = blockConvert;
        }
    }
}
