...
namespace ExtenderApp.Common.IO
{
    ...
    {
        ////可选择类型
        ////XMLTextbuffer------提供以快速、单向、无缓冲的方式存取XML数据。（单向意味着你只能从前往后读取XML文件，而不能逆向读取）

        ////XMLValidatingbuffer------与XMLTextbuffer类一起使用，提供验证DTD、XDR和XSD架构的能力。

        ////XMLDocument------遵循W3C文档对象模型规范的一级和二级标准，实现XML数据随机的、有缓存的存取。
        ////一级水平包含了DOM的最基本的部分，而二级水平增加多种改进，包括增加了对名称空间和级连状图表(CSS)的支持。

        ////XMLTextbuffer------生成遵循 W3C XML 1.0 规范的XML文件。
        ...
        //    // 保存 XML 文件
        //    //文档一些设置
        //    XmlbufferSettings settings = new XmlbufferSettings();
